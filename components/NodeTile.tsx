...
export function NodeTile({
  ...
  const kindValue = step.kind ?? "process";
  const kind = kindMeta(kindValue, resolvedTheme);
  const showKind = kindValue !== "none";
  const showHeader = showKind || !!actorName;
  const bareNone = kindValue === "none" && !showHeader;
  ...
  const bodyTint = bareNone ? (lightMode ? "24" : "26") : lightMode ? "08" : "0d";
  const bodyFadeTint = bareNone
    ? lightMode
      ? "12"
      : "18"
    : lightMode
  ...
  return (
      ...
      style={{
        ...
        borderColor: selected
          ...
          : connectSource
            ? connectColor
            : bareNone
              ? withAlpha(accent, lightMode ? "78" : "68")
              : "var(--app-node-border)",
        background: bareNone
        ...
        boxShadow: `${
          ...
          bareNone ? withAlpha(accent, lightMode ? "30" : "24") : "var(--app-inset)"
        }${
          dragging
    ...
    >
      ...
      <h3
        className={`line-clamp-2 px-3.5 text-[13.5px] font-semibold leading-snug text-text ${
          showHeader ? "pt-2.5" : "pt-4"
        }`}
      >
        {step.title}
      </h3>
